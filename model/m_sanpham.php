...
class M_sanpham extends database
{
    ...
    function updateProduct($id, $arrList)
    {
        $tieude = $arrList[0];
        $idLoaiSanPham = $arrList[1];
        $tomtat = $arrList[2];
        $noidung = $arrList[3];
        $gia = $arrList[4];
        $sql = "UPDATE sanpham SET tieude=$tieude, idLoaiSanPham=$idLoaiSanPham, tomtat=$tomtat, noidung=$noidung, gia=$gia WHERE id=$id";
        $this->setQuery($sql);
        $this->execute(array($tieude, $idLoaiSanPham, $tomtat, $noidung, $gia));
    }
